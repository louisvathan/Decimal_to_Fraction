...
rounding = 0 # ["Round to the Nearest Fraction", "Round to next Fraction", "Round to Previous Fraction", "No rounding"]

middle_condition = 0 # {"0":"to Next", "1":"to Previous"}

denominator = 8 # [64, 32, 16, 8, 4, 2, 1]
...
while True:
    try:
        x = float(input("Enter any decimal Value: "))
        break
    except ValueError:
        print("Input should be a decimal number. Please try again.")
        continue
...
def d2f(num, data, denominator, rounding, middle_condition):
    ...
    for i, key in enumerate(data):
        if float(key)-num == 0:
            ...
            if rounding == 0:   # Round to the nearest fraction
                if num-prev_num > next_num-num:
                    return (data[key][-1], "Done")
                elif num-prev_num < next_num-num:
                    return (data[str(prev_num)][-1], "Done")
                else: # num-prev_num == next_num-num
                    if middle_condition == 0: # next
                        return (data[key][-1], "Done")
                    else:
                        return (data[str(prev_num)][-1], "Done")
            elif rounding == 1:     # Round to the next fraction.
                return (data[key][-1], "Done")
            elif rounding == 2:
                return (data[str(prev_num)][-1], "Done")
            elif rounding == 3:     # No Rounding
...
#print(f"{x} is converted as {conv_num}.")
print(conv_num)
print(status)
